...
import { CATEGORY_META, CLEARANCE_META, isClassified } from "@/lib/lore-meta";
import type { LoreCategory, ClearanceLevel } from "@/lib/lore-meta";
...
export function ClearanceBadge({ level }: { level: ClearanceLevel }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 border px-1.5 py-0.5 text-mono text-[10px] uppercase tracking-[0.16em] ${tone}`}>
...
export function CategoryBadge({ category }: { category: LoreCategory }) {
  ...
  return (
    <span className="hud-label">{meta?.label ?? category}</span>
  );
}
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-3 text-display text-lg font-semibold leading-tight text-foreground group-hover:text-cyan">
        {classified ? <span className="bg-foreground/90 px-1 text-background">{title}</span> : title}
      </h3>
      {subtitle && (
        <p className="mt-1 text-sm text-muted-foreground">{subtitle}</p>
      )}
      {summary && (
        <p className="mt-3 line-clamp-3 text-sm text-foreground/70">{summary}</p>
      )}
